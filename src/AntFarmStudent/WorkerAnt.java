...
public class WorkerAnt extends Critter implements Processable
{
    ...
    public WorkerAnt()
    {
		...
		foodQuantity = 0;
		setDirection((int) (Math.random() * 45));
		foodLoc = null;
		queenLoc = null;
    }
    ...
    {
		for (Actor a : actors) {
			Processable processor = (Processable)a;
    ...
    public ArrayList<Location> getMoveLocations()
    {
		...
		list.add(getLocation().getAdjacentLocation(direction));
		...
		list.removeIf(location -> !getGrid().isValid(location) || getGrid().get(location) != null);
